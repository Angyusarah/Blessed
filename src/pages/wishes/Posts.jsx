...
const Posts = () => {
  ...
  useEffect(() => {
    getWish().then((res) => {
      ...
      const sortedItems = itemData.map((item, index) => {
        return {
          ...item,
          milliSeconds: new Date(item.createdAt).getTime()
        }
      }).sort(function (a, b) { return b.milliSeconds - a.milliSeconds })
      setDisplayWish(sortedItems)
    })
  }, [])
  ...
  const pageSize = 5;
  const pages = totalPosts % pageSize === 0
    ? Math.floor((totalPosts / pageSize))
  ...
  return (<>
    ...
    {displayWish && postsPerPage.map((item, i) => (
      <div className="flex  flex-col gap-1 justify-center items-start border-b-[2px] shadow-sm p-5" key={i}>
        <h2 className=" font-semibold text-xl">{item.fullName}</h2>
